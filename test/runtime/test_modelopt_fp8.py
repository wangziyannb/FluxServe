...
@pytest.mark.parametrize(
    "quantization_config",
    [
        {**STATIC_FP8_CONFIG, "quant_algo": "NVFP4"},
        {
            **STATIC_FP8_CONFIG,
            "config_groups": {
                "group_0": {
...
def test_rejects_unsupported_quantization(quantization_config):
    ...
    with pytest.raises(ValueError, match="only supports ModelOpt serialized static"):
...
def test_explicit_fp8_requires_checkpoint_metadata():
    ...
    with pytest.raises(ValueError, match="requires serialized ModelOpt FP8 metadata"):
